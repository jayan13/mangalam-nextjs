...
const CATEGORY_ID = 8163; // Podcasts

export const metadata = {
  title: 'Podcasts - Mangalam',
  description: 'Latest podcasts and audio updates from Mangalam.'
};

const getCachedInitialPosts = unstable_cache(
  async () => getInitialPosts(CATEGORY_ID),
  () => [`my-app-podcasts-posts`],
  { revalidate: 360 }
);
...
async function getInitialPosts(category_id) {
  try {
    ...
    console.error('Database error in getInitialPosts (Podcasts):', error);
    return [[]];
  }
}
