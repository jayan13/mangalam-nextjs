...
function Newimg(props) {
  ...
  return ((newsimage.file_name != null) ? <Image src={imageSrc} alt={newsimage.alt} width={w} height={h} loading="lazy" onError={() => setImageSrc("/uploads/noimg.svg")} unoptimized={process.env.NEXT_PUBLIC_IMAGE_URL.includes('mangalam.cms')} /> : <div>No Image</div>);
}
...
function Blocks(props) {
  ...
  return (
    <div className="news-item" >
      <figure> <Link href={`/en-news/${ndata.url}`}><Newimg news={ndata} width='213' height='128' /></Link></figure>
      <div className="news-item-text">
        <h3>
          <Link href={`/en-news/${ndata.url}`}> {ndata.title}</Link>
        </h3>
  ...
  for (let nws in Object.keys(ndata)) {
    if (i == 0) {
      rcnt0 = <div className="news-item">
        <figure> <Link href={`/en-news/${ndata[nws].url}`}><Newimg news={ndata[nws]} width='500' height='300' /></Link></figure>
        <div className="news-item-text">
          <h1>
            <Link href={`/en-news/${ndata[nws].url}`}> {ndata[nws].title}</Link>
          </h1>
    ...
    if (i == 1) {
      rcnt1 = <div className="news-item">
        <h3><Link href={`/en-news/${ndata[nws].url}`}> {ndata[nws].title}</Link>  </h3>
        <figure><Link href={`/en-news/${ndata[nws].url}`}><Newimg news={ndata[nws]} width='88' height='54' /></Link> </figure>
      </div>;
    }
    if (i == 2) {
